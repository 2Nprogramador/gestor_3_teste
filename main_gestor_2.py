...
# Função para carregar dados de um arquivo CSV carregado pelo usuário
def carregar_dados(arquivo):
    if arquivo is not None:
        return pd.read_csv(arquivo)
    return pd.DataFrame()

# Carregar dados de saída de vendas dos arquivos CSV
vendas_file = st.file_uploader("Upload das vendas", type="csv", key="vendas")
vendas_df = carregar_dados(vendas_file) if vendas_file else pd.DataFrame(columns=["Código da Venda", "Produto", "Lote", "Quantidade", "Método de Pagamento", "Data da Venda", "Valor Unitário (R$)", "Valor Total (R$)"])

# Carregar dados de entrada de estoque dos arquivos CSV
estoque_file = st.file_uploader("Upload do registro de estoque", type="csv", key="estoque")
registro_estoque_df = carregar_dados(estoque_file) if estoque_file else pd.DataFrame(columns=["Produto", "Lote", "Quantidade", "Data de Entrada", "Data de Validade", "Custo (R$)", "Valor de Venda (R$)"])
...
def salvar_dados():
    ...
    # Salvando DataFrames no armazenamento local
    vendas_df.to_csv("vendas_salvas.csv", index=False)
    registro_estoque_df.to_csv("registro_estoque_salvo.csv", index=False)
...
def entrada_estoque():
    ...
    produto = st.text_input("Nome do Produto", key="produto").upper()
    quantidade = st.number_input("Quantidade", min_value=0, step=1, key="quantidade")
    data_entrada = datetime.today().date()
    data_validade = st.date_input("Data de Validade", key="data_validade")
    custo = st.number_input("Custo do Produto (R$)", key="custo")
    valor_venda = st.number_input("Valor de Venda (R$)", key="valor_venda")
    ...
    if st.button("Adicionar ao Estoque", key="adicionar_estoque"):
...
def saida_vendas():
    ...
    produtos_selecionados = st.multiselect("Selecione os Produtos", produtos_ordenados["Produto"] + " - " + produtos_ordenados["Lote"], key="produtos_selecionados")
...
def visualizar_dados():
    ...
    mostrar_informacoes_negocio = st.sidebar.checkbox("Mostrar Informações do Negócio", value=False, key="mostrar_informacoes_negocio")
...
page = st.sidebar.radio("Selecione uma opção", options=["Entrada de Estoque", "Saída de Vendas", "Visualizar Dados"], key="pagina")
